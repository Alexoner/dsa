...
double calc_num(char* infix, double* result)
{
    ...
    if (infix[i - 2] >= '0' && infix[i - 2] <= '9') { //convert the last string to number
        str[j] = '\0';
        opnd[++topd] = atof(str);
    ...
    while (topt != -1) { //deal with operators still in stack
        if (topd) {      //must have at least two operands
